...
const BinarySearch = () => {
  ...
  const handleReset = () => {
    ...
    setIsAnimating(false); setIsPaused(false); setPendingStart(false);
  ...
  const animateBinarySearch = useCallback(() => {
    ...
    if (l > h) {
      ...
      setStepExplanation(`Search range exhausted (low > high). The element ${targetValue} does not exist in this array.`);
    ...
    setI(l); setJ(h); setMid(m); setStepCount(currentStep);
    setStepExplanation(`Step ${currentStep}: low=${l}, high=${h} → mid = ⌊(${l} + ${h}) / 2⌋ = ${m}. Comparing arr[${m}] = ${arr[m]} with target ${targetValue}.`);
    ...
    animationRef.current = setTimeout(() => {
      if (arr[m] === targetValue) {
        ...
        setStepExplanation(`✓ arr[${m}] = ${arr[m]} equals target ${targetValue}. Found at index ${m} after ${currentStep} step${currentStep > 1 ? "s" : ""}!`);
        setIsAnimating(false);
        gsap.to(elementRefs.current[m], { backgroundColor: "#22C55E", borderColor: "#15803D", duration: 0.3 });
      } else if (arr[m] < targetValue) {
        setStepExplanation(`arr[${m}] = ${arr[m]} < target ${targetValue} → target is in the RIGHT half. Discard left side. New low = ${m + 1}.`);
        searchStateRef.current.l = m + 1;
        animationRef.current = setTimeout(() => { if (!isPausedRef.current) animateBinarySearch(); }, delay * 0.6);
      } else {
        setStepExplanation(`arr[${m}] = ${arr[m]} > target ${targetValue} → target is in the LEFT half. Discard right side. New high = ${m - 1}.`);
  ...
  const handleGo = (e) => {
    ...
    if (!arrayElements || !target) { setMessage("Please fill in all fields."); setMessageType("warning"); return; }
    ...
    if (!isSorted) { setMessage("Array must be sorted in ascending order."); setMessageType("warning"); return; }

    searchStateRef.current = { l: 0, h: elements.length - 1, arr: elements, targetValue, step: 0 };
    setArray(elements); setI(0); setJ(elements.length - 1);
    setIsAnimating(true); setIsPaused(false);
    isPausedRef.current = false;
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.code === "Space" && isAnimatingRef.current &&
  ...
  const messageClass =
    ...
    : messageType === "warning" ? "bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200"
  ...
  return (
    <main className="container mx-auto">
      ...
      <form ref={formRef} onSubmit={handleGo}
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2">
            <input type="text" id="arrayElements" value={arrayElements}
              ...
              placeholder="e.g., 1, 3, 4, 6, 8" disabled={isAnimating} />
            <button type="button" onClick={generateRandomArray}
              className="px-4 py-2 font-bold bg-[#a435f0] text-white rounded-lg hover:bg-[#8f2cd6] transition-all duration-200"
              disabled={isAnimating}>Random</button>
          </div>
        </div>
        <div className="mb-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end gap-4">
            <input type="number" id="target" value={target}
              ...
              placeholder="eg. 4" disabled={isAnimating} />
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between mb-4 bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl border border-gray-200 dark:border-gray-700 gap-4">
            ...
            <div className="flex items-center gap-3">
              <button type="button" onClick={decreaseSpeed}
                ...
                disabled={speed <= 0.5}>-</button>
              <span className="text-gray-700 dark:text-gray-300 font-medium min-w-[80px] text-center">Speed: {speed}x</span>
              <button type="button" onClick={increaseSpeed}
                className="bg-white dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-300 dark:border-gray-600 px-4 py-2 rounded-lg transition-colors shadow-sm"
                disabled={speed >= 5}>+</button>
            </div>
          </div>
        )}
      </form>
          ...
          {stepExplanation && (
            <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
              <div className="flex items-center gap-2 bg-[#a435f0]/10 dark:bg-[#a435f0]/20 px-4 py-2 border-b border-[#a435f0]/20">
                ...
                {stepCount > 0 && (
                  <span className="ml-auto text-xs text-gray-500 dark:text-gray-400">Iteration #{stepCount}</span>
                )}
              </div>
              ...
              {array.map((element, index) => {
                ...
                return (
                  <div key={index} className="flex flex-col items-center">
                    <div ref={(el) => (elementRefs.current[index] = el)}
                    ...
                    <div className="mt-1 text-xs text-gray-600 dark:text-gray-400 text-center font-mono">
                      {labels.map((label, idx) => (
                        <div key={idx} className={
                          label === "mid" ? "text-yellow-600 dark:text-yellow-400 font-semibold"
                          : label === "low" || label === "high" ? "text-blue-500 dark:text-blue-400 font-semibold" : ""
                        }>{label}</div>
                      ))}
